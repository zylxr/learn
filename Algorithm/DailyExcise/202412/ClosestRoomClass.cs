...
namespace Algorithm.DailyExcise
{
    public class ClosestRoomClass
    {
        ...
        public int[] ClosestRoom(int[][] rooms, int[][] queries)
        {
            ...
            for (var i = 0; i < m; i++) events.Add(new Event(0, rooms[i][1], rooms[i][0],i));
            for(var j = 0;j<n;j++)events.Add(new Event(1, queries[j][1], queries[j][0],j));
            events.Sort();
            var ans = new int[n];
            Array.Fill(ans, -1);
