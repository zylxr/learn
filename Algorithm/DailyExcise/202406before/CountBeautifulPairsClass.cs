...
{
    public class CountBeautifulPairsClass
    {
        ...
        {
            var first = int.Parse(x.ToString().Substring(0, 1));
            var last = int.Parse(y.ToString().Substring(y.ToString().Length - 1));
            var min = Math.Min(first, last);
            var g = 1;
            for (var i = 1; i <= min; i++)
            {
                if (first % i == 0 && last % i == 0) g = i;
