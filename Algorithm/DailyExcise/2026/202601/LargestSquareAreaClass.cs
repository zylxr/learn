...
{
    public class LargestSquareAreaClass
    {
        //3047. 求交集区域内的最大正方形面积
        ...
        public long LargestSquareArea(int[][] bottomLeft, int[][] topRight)
        {
            ...
            for(var i=0;i<n;i++)
            {
                for(var j=i+1;j<n;j++)
                {
                    var w = Math.Min(topRight[i][0], topRight[j][0]) - Math.Max(bottomLeft[i][0], bottomLeft[j][0]);
                    var h = Math.Min(topRight[i][1], topRight[j][1]) - Math.Max(bottomLeft[i][1], bottomLeft[j][1]);
