...
{
    public class MinimumCostClass8
    {
        //2977. 转换字符串的最小成本 II
        ...
        public long MinimumCost(string source, string target, string[] original, string[] changed, int[] cost)
        {
            ...
            for(var k=0;k<size;k++)
            {
                ...
                {
                    for (var j = 0; j < size; j++) G[i, j] = Math.Min(G[i, j], G[i, k] + G[k, j]);
                }
            }
            var f = new long[n];
            Array.Fill(f, -1);
            for(var j=0;j<n;j++)
            {
                ...
                if (source[j] == target[j]) Update(ref f[j], baseVal);
                var u = root;
                var v = root;
                for(var i=j;i<n;i++)
                {
                    ...
                    {
                        var newval = baseVal + G[u.id, v.id];
