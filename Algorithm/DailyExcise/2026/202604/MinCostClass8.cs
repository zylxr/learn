...
namespace Algorithm.DailyExcise
{
    public class MinCostClass8
    {
        ...
        public int MinCost(int[] startPos, int[] homePos, int[] rowCosts, int[] colCosts)
        {
            ...
            var c2 = homePos[1];
            var res = 0;
            if (r2 >= r1) for (var i = r1 + 1; i <= r2; i++) res += rowCosts[i];
            else for (var i = r2; i < r1; i++) res += rowCosts[i];
            if (c2 >= c1) for (var i = c1 + 1; i <= c2; i++) res += colCosts[i];
            else for (var i = c2; i < c1; i++) res += colCosts[i];
            return res;
        }
    }
}
