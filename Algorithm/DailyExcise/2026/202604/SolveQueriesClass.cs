...
{
    public class SolveQueriesClass
    {
        //3488. 距离最小相等元素查询
        ...
        public IList<int> SolveQueries(int[] nums, int[] queries)
        {
            ...
            for (var i = 0; i < m; i++)
            {
                var num = nums[queries[i]];
                if (cnt[num].Count < 2) res.Add(-1);
                else
                {
                    var t = int.MaxValue;
                    foreach (var j in cnt[num])
                    {
                        if (j == queries[i]) continue;
                        t = Math.Min(t, Math.Abs(j - queries[i]));
                        t = Math.Min(t,Math.Min(Math.Abs(n -j + queries[i]), Math.Abs(n + j - queries[i])));
                    }
                    if (t == int.MaxValue) t = -1;
                    res.Add(t);
                }
            }
            return res;
