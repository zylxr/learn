...
{
    public class GenerateStringClass
    {
        //3474. 字典序最小的生成字符串
        ...
        public string GenerateString(string str1, string str2)
        {
            var n = str1.Length;
            var m = str2.Length;
            var s = new char[m + n - 1];
            var fixed_ = new int[m + n - 1];
            for (var i = 0; i < s.Length; i++) s[i] = 'a';
            for(var i=0;i<n;i++)
            {
                if (str1[i] == 'T')
                {
                    ...
                    {
                        {
                            if (fixed_[j] == 1 && s[j] != str2[j - i]) return "";
                            else
                            {
                                s[j] = str2[j - i];
                                fixed_[j] = 1;
