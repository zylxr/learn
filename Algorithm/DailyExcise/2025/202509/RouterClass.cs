...
namespace Algorithm.DailyExcise
{
    public class RouterClass
    {
        ...
        private HashSet<Packet> packetSet = new();
        private readonly Dictionary<int, Pair> distToTimeStamps = new();
        public RouterClass(int memoryLimit)
        {
        ...
        public bool AddPacket(int source, int destination, int timestamp)
        {
            ...
            if (!distToTimeStamps.ContainsKey(destination)) distToTimeStamps[destination] = new Pair();
            distToTimeStamps[destination].TimeStamps.Add(timestamp);
            return true;
        }
        public int[] ForwardPacket()
        {
            ...
            distToTimeStamps[packet.Destination].Head++;
            return new int[] { packet.Source,packet.Destination,packet.TimeStamp};
        }

        public int GetCount(int destination, int startTime, int endTime)
        {
            if (!distToTimeStamps.ContainsKey(destination)) return 0;
            var p = distToTimeStamps[destination];
            var left = LowerBound(p.TimeStamps, startTime, p.Head);
            var right = LowerBound(p.TimeStamps, endTime + 1, p.Head);
            return right - left;
        }
