...
{
    public class FindAllPeopleClass
    {
        //2092. 找出知晓秘密的所有专家
        ...
        public IList<int> FindAllPeople(int n, int[][] meetings, int firstPerson)
        {
            ...
            for(var i=0;i<m;)
            {
                ...
                for(var k=i;k<=j;k++)
                {
                    ...
                    vertices.Add(x);
                    vertices.Add(y);
                    if (!edges.ContainsKey(x)) edges[x] = new List<int>();
                    if(!edges.ContainsKey(y)) edges[y] = new List<int>();
                    edges[x].Add(y);
                    edges[y].Add(x);
                }
                var q = new Queue<int>();
                foreach (var u in vertices) if (visited[u]) q.Enqueue(u);
                while(q.Count > 0)
                {
                    var u = q.Dequeue();
