...
{
    public class MaxFrequencyClass
    {
        //3346. 执行操作后元素的最高频率 I
        ...
        public int MaxFrequency(int[] nums, int k, int numOperations)
        {
            ...
            for (var i = 0; i < nums.Length; i++)
            {
                ...
                {
                    cnt[nums[lastNumIndex]] = i - lastNumIndex;
                    ans = Math.Max(ans, i - lastNumIndex);
                    lastNumIndex = i;
                }
            }
            cnt[nums[lastNumIndex]] = nums.Length- lastNumIndex;
            ...
            for (var i = nums[0]; i <= nums[nums.Length - 1];i++)
             {
                ...
                var tmpAns = 0;
                if (cnt.ContainsKey(i)) tmpAns = Math.Min(r - l + 1, cnt[i] + numOperations);
                else tmpAns = Math.Min(r - l + 1, numOperations);
