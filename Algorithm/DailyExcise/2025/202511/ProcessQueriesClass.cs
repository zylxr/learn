...
{
    public class ProcessQueriesClass
    {
        //3607. 电网维护
        ...
        //queries[i].length == 2
        //queries[i][0] 为 1 或 2。
        //1 <= queries[i][1] <= c给你一个整数 c，表示 c 个电站，每个电站有一个唯一标识符 id，从 1 到 c 编号。
        ...
        {
            var dsu = new DSU(c + 1);
            foreach (var p in connections) dsu.Join(p[0], p[1]);
            var online = new bool[c + 1];
            var offlineCounts = new int[c + 1];
            Array.Fill(online, true);
            var minimuOnlineStations = new Dictionary<int, int>();
            ...
            {
                var root = dsu.Find(i);
                if (!minimuOnlineStations.ContainsKey(root)) minimuOnlineStations[root] = -1;
                var station = minimuOnlineStations[root];
                if (online[i])
                {
                    if (station == -1 || station > i)
                        minimuOnlineStations[root] = i;
                }
            }
            ...
            for(var i=queries.Length-1;i>=0;i--)
            {
                ...
                if(op==2)
                {
                    ...
                    {
                        online[x] = true;
                        if (station == -1 || station > x) minimuOnlineStations[root] = x;
                    }
                }
            }
            ans.Reverse();
