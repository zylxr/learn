...
{
    public class EarliestAndLatestClass
    {
        //1900. 最佳运动员的比拼回合
        ...
        {
            if (F[n, f, s] != 0) return new int[] { F[n, f, s], G[n,f,s] };
            if (f + s == n + 1) return new[] { 1, 1 };
            if(f+s>n+1)
            {
                var res = Dp(n, n + 1 - s, n + 1 - f);
                F[n, f, s] = res[0];
                G[n,f,s] = res[1];
                return new int[] { F[n, f, s], G[n,f,s] };
            }
            var earlist = int.MaxValue;
            ...
            }else
            {
                ...
                for(var i=0;i<f;i++)
                {
                    for(var j=0;j<s_prime-f;j++)
                    {
                        var res = Dp(n_half, i + 1, i + j + mid + 2);
                        earlist = Math.Min(earlist, res[0]);
                        latest = Math.Max(latest, res[1]);
                    }
                }
